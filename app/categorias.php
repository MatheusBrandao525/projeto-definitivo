...
<div class="container" id="produtos" style="margin-top: 3px;">
        ...
        <?php  while($exibir = $consulta->fetch(PDO::FETCH_ASSOC)) { ?>
            ...
            <div class="col-sm-2" style="margin-bottom: 8px;">
                ...
                <a class="btn btn-info btn-block btndetalhes" href="../app/detalhes.php?id=<?php echo $exibir['id_produto']; ?>">Detalhes</a> <!--Botão de detalhes-->
                ...
                <?php if(empty($_SESSION['ID'])) { ?> <!--Verificando, se a sessão ID estiver vazia entao..-->
                    <?php if($exibir['qnt_estoque'] > 0) {?> <!--Verificando se a quantidade do produto em estoque é maior que zero-->
                        <a href="../app/login.php" style="text-decoration:none;"> <!--Se a sessão ID não estiver setada entao o usuario ao clicar em adicionar ao carrinho é redirecionado para a pagina de login-->
                ...
                <?php } else { ?>
                    <?php if($exibir['qnt_estoque'] > 0) {?> <!--Verificando se a quantidade do produto em estoque é maior que zero-->
                        ...
                        <a href="../app/carrinho.php?id=<?php echo $exibir['id_produto'];?>" style="text-decoration:none;">
                            <button type="button" class="btn btn-success btn-block">Adicionar ao Carrinho</button>
                        </a>
                    <?php } else { ?>
